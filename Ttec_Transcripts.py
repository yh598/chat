...
processing_start_time_ttec = datetime.now()

# COMMAND ----------

source_directory_ttec = "/Volumes/prod_adb/default/synapse_volume/raw/surveyspeechextraction/TTEC/"

target_directory_ttec = "/Volumes/dbc_adv_anlaytics_dev/surveyspeechextraction/call_intent_workspace/common_filter/"
...
unzip_path_ttec=unzip_ttec_files(source_directory_ttec,original_zip_files, raw_unzip_path, folder_to_execute)
...
processing_end_time_ttec=datetime.now()
...
def update_daily_transcript_logs(metadata_table_name, processing_start_time, processing_end_time):
    try:
        ...
        filtered_metadata = metadata_table.filter(
            (metadata_table.CII_Source.like('%TTec%')) &
        ...
        lob_counts = lob_counts.withColumn("Date_Created", current_date()) \
            ...
            .withColumn("Vendor_Information", lit("TTec")) \
...
update_daily_transcript_logs("metadata_table", processing_start_time_ttec, processing_end_time_ttec)